...
public enum Structure {
    ...
    BARRACK("Barrack", false, R.drawable.barrack, new Item[]{new Item(GameResource.BARRACK_BP, 1)}) {
        @Override
        public void applyBonus(PlaceBonuses bonuses) {
            bonuses.addCapacity(5); //The place gains 5 more capacity to hold more units
        }
    },
    STORAGE("Storage", false, R.drawable.storage, new Item[]{new Item(GameResource.STORAGE_BP, 1)}) {
        @Override
        public void applyBonus(PlaceBonuses bonuses) {
            bonuses.addResourceBonus(0.2); //The players whose faction owns the place get +20% of resources when collecting them;
        }
    },
    VAULT("Vault", false, R.drawable.vault, new Item[]{new Item(GameResource.VAULT_BP, 1)}) {
        @Override
        public void applyBonus(PlaceBonuses bonuses) {
            bonuses.addResourcePenalty(0.1); //The players will get -10% of resources when stealing from a place owned by the enemy;
        }
    },
    ARMORY("Armory", false, R.drawable.armory, new Item[]{new Item(GameResource.ARMORY_BP, 1)}) {
        @Override
        public void applyBonus(PlaceBonuses bonuses) {
    ...
    MEDBAY("Medbay", false, R.drawable.medbay, new Item[]{new Item(GameResource.MEDBAY_BP, 1)}) {
        @Override
        public void applyBonus(PlaceBonuses bonuses) {
            bonuses.addHealth(100); //Units will get +100 health points
        }
    },
    LABORATORY("Laboratory", false, R.drawable.laboratory, new Item[]{new Item(GameResource.LABORATORY_BP, 1)}) {
        @Override
        public void applyBonus(PlaceBonuses bonuses) {
